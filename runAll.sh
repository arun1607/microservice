...
if [ $order_service_input == "y" ]; then
	if [ -f order_management_artifact/PID ]; then
		...
		java -jar ./order_management_artifact/target/inventory-management-1.2.6.RELEASE.jar -Dcom.sun.management.jmxremote.port=1102 -Dcom.sun.management.jmxremote.authenticate=false > inventory_management.log 2>&1 &
...
if [ $shipment_service_input == "y" ]; then
	if [ -f shipment_management_artifact/PID ]; then
		...
		java -jar ./shipment_management_artifact/target/inventory-management-1.2.6.RELEASE.jar -Dcom.sun.management.jmxremote.port=1102 -Dcom.sun.management.jmxremote.authenticate=false > inventory_management.log 2>&1 &
